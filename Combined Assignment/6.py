...
while True:
    ...
    if choice == 'exit':
        ...
        options = input(
            "Type Y if you want to see all the information of the employee. Type N if you want to see a particular field ").split()
        option = options[0].strip().lower()
        if option == 'y':
            print(EmployeeData[choice])
        elif option == 'n':
            field = input(
                "Enter the field which you want to see  -> name , job , DateOfJoining , salary  ").split()
